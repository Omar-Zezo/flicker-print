...
import { checkOtpCode } from "@/store/slices/auth/checkOtp";
...
const VerifyForm = () => {
  ...
  const { data, error } = useSelector((state) => state.checkOtp);
    ...
    dispatch(
      checkOtpCode({
        data: {
          ...
          otp: `${data.input1}${data.input2}${data.input3}${data.input4}${data.input5}${data.input6}`,
        },
      })
    );
  };
